...
class TestTokenspeedMLANoPEPrefill(CustomTestCase):
    """NoPE MLA layers take the rope-free branch of the FP8 prefill hook.

    Kimi Linear builds its MLA layers with ``skip_rope=True``, which leaves
    ``rotary_emb`` as ``None``. ``prepare_prefill_qkv`` must then stay clear of
    the fused RoPE kernel, still hand the FMHA kernel packed ``[nope | pe]``
    FP8 Q/K plus FP8 V, and write the unrotated ``k_pe`` to the KV cache.
    Only Triton kernels run on this branch, so no Blackwell GPU is needed.
    """
    ...
    def test_nope_prefill_skips_rope_and_packs_fp8(self):
        ...
        # Shaped the way forward_normal_prepare hands them over: q is the full
        # [nope | pe] projection and q_pe its trailing view, kv_a the normed
        # latent, k_pe the strided tail of the latent cache.
        ...
        # The KV cache gets the FP8 latent plus the unrotated k_pe in the
        # [tokens, 1, qk_rope] layout the decode path reads back.
